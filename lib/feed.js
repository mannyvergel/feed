...
function Feed(options) {
    ...
    this.addItem = function (options) {
        ...
        var item = {
            ...
            'media:thumbnail': options['media:thumbnail']
        };
...
function atom_1_0(options) {
    ...
    for(var i = 0; i < entries.length; i++) {
        ...
        if (entries[i]['media:thumbnail']) {
            entry.push({ 'media:thumbnail': { _attr: { 'xmlns:media':'http://search.yahoo.com/mrss/', 'url': entries[i]['media:thumbnail'], height: 72, width: 72}} });
        }
        // entry author(s)
        if(entries[i].author) {
